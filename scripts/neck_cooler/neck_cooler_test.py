...
def control_peltier_and_fan():
    try:
        while True:
            user_input = input("左右のペルチェのデューティー比 (-100から100) をスペースで区切って入力: ")
            ...
            try:
                left_duty, right_duty = map(float, user_input.split())
            except ValueError:
...
def main():
    try:
        control_thread = threading.Thread(target=control_peltier_and_fan)
